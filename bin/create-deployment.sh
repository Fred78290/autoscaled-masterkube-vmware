...
kubectl create configmap config-cluster-autoscaler --dry-run=client -n kube-system -o yaml \
	...
	--from-file ${TARGET_CONFIG_LOCATION}/kubernetes-vmware-autoscaler.json | kubectl apply -f -

kubectl create configmap kubernetes-pki --dry-run=client -n kube-system -o yaml \
	--kubeconfig=${TARGET_CLUSTER_LOCATION}/config \
	--from-file ${TARGET_CLUSTER_LOCATION}/kubernetes/pki | kubectl apply -f -

if [ "${EXTERNAL_ETCD}" = "true" ]; then
    kubectl create secret generic etcd-ssl --dry-run=client -n kube-system -o yaml \
		--kubeconfig=${TARGET_CLUSTER_LOCATION}/config \
        --from-file ${TARGET_CLUSTER_LOCATION}/etcd/ssl | kubectl apply -f -
else
    kubectl create secret generic etcd-ssl --dry-run=client --kubeconfig=${TARGET_CLUSTER_LOCATION}/config -n kube-system -o yaml \
        --from-file ${TARGET_CLUSTER_LOCATION}/kubernetes/pki/etcd | kubectl apply -f -
fi
...
NGINX_IP=$(kubectl get svc ingress-nginx-controller -n ingress-nginx --kubeconfig=${TARGET_CLUSTER_LOCATION}/config -o yamlpath='{.status.loadBalancer.ingress[0].ip}')
...
# Save template
kubectl create ns ${NODEGROUP_NAME} --dry-run=client --kubeconfig=${TARGET_CLUSTER_LOCATION}/config -o yaml | kubectl apply -f -

# Add cluster config in configmap
kubectl create configmap cluster --dry-run=client -n ${NODEGROUP_NAME} -o yaml \
    --kubeconfig=${TARGET_CLUSTER_LOCATION}/config \
	--from-file ${TARGET_CLUSTER_LOCATION}/ | kubectl apply -f -

kubectl create configmap config --dry-run=client -n ${NODEGROUP_NAME} -o yaml \
    --kubeconfig=${TARGET_CLUSTER_LOCATION}/config \
	--from-file ${TARGET_CONFIG_LOCATION} | kubectl apply -f -

# Save deployment template
pushd ${CONFIGURATION_LOCATION}/deployment
	for DIR in $(ls -1 -d */ | tr -d '/')
	do
		kubectl create configmap ${DIR} --dry-run=client -n ${NODEGROUP_NAME} -o yaml \
			--kubeconfig=${TARGET_CLUSTER_LOCATION}/config --from-file ${DIR} | kubectl apply -f -
	done
popd
